...
- [ ] Метрики совпадают с эталонным ручным расчётом на фикстуре.
- [ ] Sharpe сравним между разными timeframes (корректный annualization).
- [ ] При `trades_count < min_trades` метрики помечаются как ненадёжные.
- [ ] `test_metrics.py` покрывает граничные случаи (0 сделок, без просадки).
